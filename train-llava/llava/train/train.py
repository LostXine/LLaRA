...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            sentence['value'] = sentence['value'].replace('<s>', '').replace('</s>', '') # add this for kk so that it will not mislead the conversation spliting
            continue
            # below is old process for image, it will be invalid because now we use a series of new tokens for images
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            sample = [sample]
        assert len(sample) == 1, "Don't know why it is wrapped to a list"  # FIXME
        if 'image' in sample[0]:
            image_list = []
            # wrap it into a list
            if isinstance(self.list_data_dict[i]['image'], str):
            ...
            for image_file in self.list_data_dict[i]['image']:
                ...
                if os.path.exists(image_full_path):
                    ...
                    assert self.data_args.zip_data_path, f'Image {image_full_path} not found on disk while the zip file is not assigned.'
                    with self.zip_file.open(image_full_path, 'r') as f:
                        image = Image.open(f)
            ...
            image = torch.stack(image_list) # B, 3, H, W
        ...
        if 'image' in self.list_data_dict[i]:
            data_dict['image'] = image # update: B, 3, H, W
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            model = LlavaLlamaForCausalLM.from_pretrained(
                ...
                torch_dtype=(torch.bfloat16 if training_args.bf16 else torch.bfloat16),
                **bnb_model_from_pretrained_args
            )
    else:
